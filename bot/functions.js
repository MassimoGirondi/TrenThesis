...
exports.showProfessors = function(chatId, jsonobj, bot) {
    ...
    for (var i = 0; i < jsonobj.length; i++) {
        ...
        if (jsonobj[i].hasOwnProperty('first_name') && jsonobj[i].hasOwnProperty('last_name') && jsonobj[i].hasOwnProperty('id'))
            professorName_id.push([jsonobj[i].first_name + " " + jsonobj[i].last_name, jsonobj[i].id]);
    }
...
exports.showThesisInfo = function(chatId, jsonobj, bot) {
    var profJson;
    var url = apiUrl+"/professors/"+jsonobj.professor_id;
    request({
        url: url,
        json: true
    }, function(err, res, json) {
        if (err) {
            ...
            var thesis = constants.THESIS +
                "\n" + constants.TITLEEMOJI + constants.TITLE + jsonobj.title +
                ...
                "\n\n" + constants.PROFINFO +
                "\n" + constants.PROFEMOJI + constants.NAME + profJson.first_name + " " + profJson.last_name +
                "\n" + constants.EMAILEMOJI + constants.EMAIL + profJson.email +
                "\n" + constants.DEPARTMENTEMOJI + constants.DEPARTMENT + profJson.department +
                "\n" + constants.WEBSITEEMOJI + constants.WEBSITE + "<a href=\"" + profJson.website + "\" >" + constants.CLICKHERE + "</a>";
